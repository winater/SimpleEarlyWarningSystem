    ...
    def parse(self, url=None):
        ...
        for li in ul.find_all(attrs={'class': 'S_line1', 'action-type': 'itemClick'}):
            ...
            for j in span.stripped_strings:
                ...
                #print templocation
                #print type(templocation)
                localvalue=open("D:\\09Limited_buffer\\earlywarningbyci\\cola\\contrib\\weibo\\localvalue.txt","r")
                filelocation=localvalue.readline().decode('utf-8')
                #print "filelocation",filelocation
                localvalue.close()
                print templocation,filelocation
                #print type(templocation),type(filelocation)
                #time.sleep(10)
                filelocation=filelocation.rstrip()
                #print "after",templocation,filelocation
                #print "after",type(templocation),type(filelocation)
                if templocation == filelocation:
                    #"u'\u6c5f\u82cf \u5357\u4eac'":
                    locationok = True
                    print "==============location====================="
                    friend.uid = tempuid
                    friend.nickname = tempnickname
                    friend.sex = tempsex
                    friend.location = templocation
                    break
            if locationok:
                ...
                weibo_user.save()
            span = span.find_next(attrs={'class': 'addr'})
            #
            #
            #
        self.logger.debug('parse %s finish' % url)
