...
def big_data_cal_affect(f,original_res,new_sc,text,first_range,ill_count):
    f.write("--------------------------"+str(text)+"------------------------------"+"\n")
    f.write(u"阈值\tF值\t召回率\t正确率=\t共找到疑似违规帖子\t真正违规\t漏掉\n")
    for threhold in first_range:
...
def main():
    ...
    if options.step==4:
        ...
        if output==False:
            ...
            f.write(u"所有类别各个阈值下得F值、召回率、准确率")
            save_result(f,rate)
